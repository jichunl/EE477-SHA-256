...
*  When it finishes, subdirectories are prepared for your blocks and top-level 
   ...
   o  Creating preliminary block abstraction (or ILM), FRAM for each block (before detailed block 
      implementation) and linking to the top-level. 

      This allows an early timing check of the top level CEL. Note: You should 
      always finish block implementation to get accurate block abstraction (or ILM) and FRAM.
...
*  To use Zroute for pin assignment, set ICC_DP_USE_ZROUTE to TRUE in icc_setup.tcl.
...
This runs IC Compiler Reference Methodology for block implementation.  At the end, it 
generates the block abstraction (or ILM) and FRAM for the block.
...
Run Makefile from top directory: make -f rm_setup/Makefile_zrt ic &
-------------------------------------------------------------------------

*  This runs IC Compiler Reference Methodology for top-level integration.

*  It is already set up to reference to the block libraries to use block abstraction (or ILM) and FRAM 
...
-------------------------------------------------------
When using the Lynx-compatible IC Compiler Hierarchical Reference Methodology in 
standalone mode, you need to set up the top and block directories parallel to the 
current working directory, where hierarchical design planning is run. Name the 
