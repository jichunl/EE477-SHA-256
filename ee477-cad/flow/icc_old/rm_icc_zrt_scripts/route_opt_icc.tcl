...
if {$ICC_IMPLEMENTATION_PHASE == "top" && $ICC_CREATE_BLOCK_ABSTRACTION} {
  ...
  if {$ICC_TIO_OPTIMIZE_BLOCK_INTERFACE && $ICC_TIO_BLOCK_LIST != "" && $ICC_TIO_HOST_OPTION_POOL_OPTION != "" && $ICC_TIO_HOST_OPTION_SUBMIT_OPTIONS != ""} {
...
if {$ICC_IMPLEMENTATION_PHASE == "top" && $ICC_CREATE_BLOCK_ABSTRACTION} {
  ...
  ## Enable -host_options if ICC_TIO_HOST_OPTION_POOL_OPTION and ICC_TIO_HOST_OPTION_SUBMIT_OPTIONS are both valid
...
########################################
#
# Verify Zroute result
if {$ICC_FIX_ANTENNA} {
  verify_zrt_route -antenna true > $REPORTS_DIR_ROUTE_OPT/$ICC_ROUTE_OPT_CEL.verify_zrt_route.rpt
} else {
  verify_zrt_route -antenna false > $REPORTS_DIR_ROUTE_OPT/$ICC_ROUTE_OPT_CEL.verify_zrt_route.rpt
}
