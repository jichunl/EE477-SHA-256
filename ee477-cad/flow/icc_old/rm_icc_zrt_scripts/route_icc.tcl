...
if {$ICC_DBL_VIA} {
  ...
  if {$ICC_DBL_VIA_FLOW_EFFORT == "HIGH"} {
    # default is off
    set_route_zrt_common_options -concurrent_redundant_via_mode reserve_spaces

    # default is low; only works if the above is not off
...
if {$ICC_REPORTING_EFFORT != "OFF" } {
  # global skew report
  redirect -tee -file $REPORTS_DIR_ROUTE/$ICC_ROUTE_CEL.clock_tree {report_clock_tree -nosplit -summary}

  # local skew report
  redirect -file $REPORTS_DIR_ROUTE/$ICC_ROUTE_CEL.clock_timing {report_clock_timing -nosplit -type skew}
}
...
if {$ICC_REPORTING_EFFORT != "OFF" } {
 # MBT I turned off nosplit -- hard to read
...
if {$ICC_FIX_ANTENNA} {
  verify_zrt_route -antenna true > $REPORTS_DIR_ROUTE/$ICC_ROUTE_CEL.verify_zrt_route.rpt
} else {
  verify_zrt_route -antenna false > $REPORTS_DIR_ROUTE/$ICC_ROUTE_CEL.verify_zrt_route.rpt
}
