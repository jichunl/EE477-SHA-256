...
########################################################################################
  ...
  # MBT: load in rp groups, before uniquification occurs, if we have not already done it
  if {[file exists results/dc/bsg_rp_groups.tcl]} {
      if {[sizeof_collection [get_rp_groups]] == 0} {
	  puts "## BSG: processing rp groups file"
	  source results/dc/bsg_rp_groups.tcl
      }
  } else {
  ...
  # MBT: leave the rp modules alone!
  set_dont_touch [get_designs bsg_rp_*]
}
...
if {$ICC_INIT_DESIGN_INPUT == "VERILOG" } {
  ...
  # MBT: we should be placing explicit timing assertions on pins rather than using this generic catchall
  # the problem with this is that it unnecessarily flags the source synchronous outputs
...
############################################
##         CREATE PLL VOLTAGE AREA        ##
############################################
if {$::env(USE_MV_FLOW) == 1} {

  set TSMC250 0

  # Need to set the operating condition for 'extract_rc' which is implicitly
  # used by many reporting commands
  #
  if {$TSMC250} {
  set_operating_conditions -library slow -max slow -min slow
  } else {
  set_operating_conditions -library sage-x_tsmc_cl018g_rvt_ss_1p62v_125c \
                           -max ss_1p62v_125c \
                           -min ss_1p62v_125c
  }
  ...
  create_power_domain $PLL_POWER_DOMAIN -elements $PLL_INSTANCE_EXP

  # Create Dummy VDD supply net
  #
  create_supply_port DMY_VDD
  create_supply_net  DMY_VDD -domain TOP
  create_supply_net  DMY_VDD -domain $PLL_POWER_DOMAIN -reuse
  connect_supply_net DMY_VDD -ports DMY_VDD

  # Create Dummy VSS supply net
  #
  create_supply_port DMY_VSS
  create_supply_net  DMY_VSS -domain TOP
  create_supply_net  DMY_VSS -domain $PLL_POWER_DOMAIN -reuse
  connect_supply_net DMY_VSS -ports DMY_VSS

  # Create VDD supply net
  #
  create_supply_port VDD
  create_supply_net  VDD -domain TOP
  create_supply_net  VDD -domain $PLL_POWER_DOMAIN -reuse
  connect_supply_net VDD -ports VDD

  # Create VSS supply net
  #
  create_supply_port VSS
  create_supply_net  VSS -domain TOP
  create_supply_net  VSS -domain $PLL_POWER_DOMAIN -reuse
  connect_supply_net VSS -ports VSS

  # Create VDD_PLL supply net
  #
  create_supply_port $PLL_POWER_NET
  create_supply_net  $PLL_POWER_NET -domain TOP
  create_supply_net  $PLL_POWER_NET -domain $PLL_POWER_DOMAIN -reuse
  connect_supply_net $PLL_POWER_NET -ports $PLL_POWER_NET

  # Create VSS_PLL supply net
  #
  #create_supply_port $PLL_GROUND_NET
  #create_supply_net  $PLL_GROUND_NET -domain TOP
  #create_supply_net  $PLL_GROUND_NET -domain $PLL_POWER_DOMAIN -reuse
  #connect_supply_net $PLL_GROUND_NET -ports $PLL_GROUND_NET

  # Set the voltage of the PLL supply nets. These supply nets
  # are not in the linked db file so they will not have operating
  # conditions and therefore 'extract_rc' will failed which is
  # implicitly used by many reporting commands
  #
  if {$TSMC250} {
  set_voltage 2.25 -object_list "$PLL_POWER_NET $PLL_GROUND_NET"
  } else {
  set_voltage 1.62 -object_list "$PLL_POWER_NET $PLL_GROUND_NET"
  }

  # Set primary power and ground for the main power domain
  #
  set_domain_supply_net TOP \
    -primary_power_net VDD \
    -primary_ground_net VSS

  # Set primary power and ground for the PLL power domain
  #
  set_domain_supply_net $PLL_POWER_DOMAIN \
    -primary_power_net $PLL_POWER_NET \
    -primary_ground_net $PLL_GROUND_NET

  # Allow derive_pg_connection to create the supply nets so that
  # it can connect the pg pins of stdcells inside the PLL va
  #
  derive_pg_connection -create_nets

  # Create the PLL voltage area
  #
  create_voltage_area \
    -power_domain $PLL_POWER_DOMAIN \
    -guard_band_x $PLL_VA_GUARD \
    -guard_band_y $PLL_VA_GUARD \
    -coordinate "$PLL_VA_LL_X $PLL_VA_LL_Y $PLL_VA_UR_X $PLL_VA_UR_Y"
}
...
save_mw_cel -as $ICC_INIT_DESIGN_CEL

########################################################################################
# Saving the cell + snapshot creation
########################################################################################
if {$ICC_REPORTING_EFFORT != "OFF" } {
  create_qor_snapshot -name $ICC_INIT_DESIGN_CEL
  redirect -file $REPORTS_DIR_INIT_DESIGN/$ICC_INIT_DESIGN_CEL.qor_snapshot.rpt {report_qor_snapshot -no_display}
}
...
if {$ICC_REPORTING_EFFORT != "OFF" } {
  ...
  ########################################################################################
  redirect -tee -file $REPORTS_DIR_INIT_DESIGN/$ICC_INIT_DESIGN_CEL.sum {report_design_physical -all -verbose}

  set_zero_interconnect_delay_mode true
  redirect -tee -file $REPORTS_DIR_INIT_DESIGN/$ICC_INIT_DESIGN_CEL.zic.qor {report_qor}
