...
if {$ICC_FLOORPLAN_INPUT == "CREATE"} {
  ...
  # Apparently initialize_floorplan is deprecated and we should use
  # create_floorplan instead.
...
if {$::env(BSG_PACKAGING_FOUNDRY) == "portable"} {
  create_floorplan \
   	-control_type aspect_ratio \
   	-core_aspect_ratio 1 \
   	-core_utilization 0.7 \
  ...
  #Get rectangular region of core placement area.
  ...
  set core_width  [expr $core_ur_x - $core_ll_x]
  set core_height [expr $core_ur_y - $core_ll_y]
  
  #STD: adding a minimum size to the core
  #
  puts "##STD- Original Core Width $core_width"
  puts "##STD- Original Core Height $core_height"
  set new_core_width $core_width
  set new_core_height $core_height
  if {$core_width < 25 } {
      set new_core_width 25
  }
  if {$core_height < 25 } {
      set new_core_height 25
  }
  adjust_fp_floorplan -core_width $new_core_width -core_height $new_core_height
  #
  #STD: END
...
if {$::env(BSG_PACKAGING_FOUNDRY) != "portable"} {
  # Move pads to their proper location
if {[file exists [which $ICC_IN_PAD_MOVE_TO_LOCATION_FILE]]} {source $ICC_IN_PAD_MOVE_TO_LOCATION_FILE}

source -echo $::env(BSG_CAD_DIR)/util/bsg/io_fp_lib.tcl

place_bond_pad -side left \
    -inner_bond_pad_ref_name "BONDPAD" \
    -outer_bond_pad_ref_name "BONDPAD" \
    -exclude_cells {corner_ll corner_lr corner_ul corner_ur}

place_bond_pad -side top \
    -inner_bond_pad_ref_name "BONDPAD" \
    -outer_bond_pad_ref_name "BONDPAD" \
    -exclude_cells {corner_ll corner_lr corner_ul corner_ur}

place_bond_pad -side right \
    -inner_bond_pad_ref_name "BONDPAD" \
    -outer_bond_pad_ref_name "BONDPAD" \
    -exclude_cells {corner_ll corner_lr corner_ul corner_ur}

place_bond_pad -side bottom \
    -inner_bond_pad_ref_name "BONDPAD" \
    -outer_bond_pad_ref_name "BONDPAD" \
    -exclude_cells {corner_ll corner_lr corner_ul corner_ur}
}
