...
redirect -file $REPORTS_DIR/$ICC_CHIP_FINISH_CEL.timing.max.rpt {report_timing -input_pins -capacitance -transition_time -nets -significant_digits 4 -attributes -nosplit -nworst 10 -max_paths 500}
redirect -file $REPORTS_DIR/$ICC_CHIP_FINISH_CEL.timing.min.rpt {report_timing -input_pins -capacitance -transition_time -nets -significant_digits 4 -attributes -nosplit -nworst 10 -max_paths 500 -delay_type min}
...
#STD: add designs report_timing.tcl
if {[file exists $::env(BSG_DESIGNS_TARGET_DIR)/tcl/report_timing.tcl]} {
    source -echo $::env(BSG_DESIGNS_TARGET_DIR)/tcl/report_timing.tcl
}
