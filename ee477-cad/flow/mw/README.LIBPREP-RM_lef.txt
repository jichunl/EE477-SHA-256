################################################################################
# Physical Library Preparation Reference Methodology for LEF_IN flow
...
The "LEF_IN" flow of Physical Library Preparation Reference Methodology includes
the following files:
----------------------------------

README.LIBPREP-RM_lef.txt
README.check_std_cell_pin_access.pdf
Release_Notes.LIBPREP-RM.txt

rm_setup/
rm_setup/libprep_setup.tcl
rm_setup/Makefile_lef_in

rm_libprep_lef_in_scripts/
rm_libprep_lef_in_scripts/analyze_lib.tcl
rm_libprep_lef_in_scripts/bpv.tcl
rm_libprep_lef_in_scripts/check_lib.tcl
rm_libprep_lef_in_scripts/check_std_cell_pin_access_amd64.tcl
rm_libprep_lef_in_scripts/check_std_cell_pin_access_linux.tcl
rm_libprep_lef_in_scripts/check_std_cell_pin_access_suse32.tcl
rm_libprep_lef_in_scripts/check_std_cell_pin_access_suse64.tcl
rm_libprep_lef_in_scripts/create_ref_libs.tcl
rm_libprep_lef_in_scripts/diode_ports.tcl
rm_libprep_lef_in_scripts/lib_db.tcl
rm_libprep_lef_in_scripts/multi_height.tcl
rm_libprep_lef_in_scripts/pr_bdry.tcl
rm_libprep_lef_in_scripts/read_lef.tcl
rm_libprep_lef_in_scripts/check_wire_tracks.tcl
rm_libprep_lef_in_scripts/wire_tracks.tcl
rm_libprep_lef_in_scripts/smash.tcl
rm_libprep_lef_in_scripts/set_port_types.tcl
rm_libprep_lef_in_scripts/extract_antenna.tcl
rm_libprep_lef_in_scripts/read_final_gds.tcl
...
Makefile_lef_in:
----------------
The Makefile is used to control the overall LEF-in flow. In most cases there 
should be no need to edit the Makefile (except for specifying the configuration 
file, see below). It only shows when you select the "LEF_IN" flow.
...
*  To run the LEF_IN flow of the reference methodology scripts, enter the 
   following command:
   
   % make -f rm_setup/Makefile_lef_in init clean complete

Should a makefile target fail, examine the log file, make the necessary 
changes to the configuration file, and re-run "make -f rm_setup/Makefile_lef_in complete". You can also
run each target by entering "make -f rm_setup/Makefile_lef_in <target>".

The makefile touch files are stored in the directory "touchfiles".

To use a specific configuration file, enter

% make -f rm_setup/Makefile_lef_in <target> FLOW_CONFIG=my_config_file
...
to bring in different types of data (in this case LEF files) and then convert 
...
init:
-----
Initializes the LEF_IN flow; deletes files created by previous operations 
...
read_lef:
---------
The advanced (manual) mode is ON by default.

Reads in the LEF file and creates the CEL view. The Milkyway Environment tool reads the LEF, converts the technology information into Milkyway format, creates a CEL view for each cell, and stores the cells into the specified Milkyway library. It also determines the standard-cell place-and-route boundaries, multiple-height place-and-route boundaries, and wire tracks of the cells. If the automated flow does not produce the desired results, start again and choose the advanced flow. 

It also creates three files with lists of cells in the LEF file -- one for standard cells, one for I/O cells, and one for other types of cells. These lists are used later in the flow.

smash:
------
Smashes (flattens) the CEL views. This is primarily required for macros and 
pads. It smashes to a depth of 20 by default. If you know that all pin shapes are on a "lower" level in the LEF file, you 
...
bpv:
----
Skip when read_lef is not advanced (manual) mode.
...
read_final_gds:
---------------
The default is ON in LEF_IN flow.

Since the CEL views are smashed and possibly pin names are modified, it is 
necessary to do a clean stream-in at the end. This final stream-in is done 
in GDSII format, even in the LEF-in flow. Upon completion, the CEL view is 
a 1:1 representation of the cell LEF, ready for stream-out.

extract_antenna:
----------------
Calculates or reads in the antenna properties for each cell in the library.
Uses IV Validator for antenna property extraction. Be aware that this may take
...
EXAMPLE
        ...
        make -f rm_setup/Makefile_lef_in analyze_lib

        Use a specific configuration file:

        make -f rm_setup/Makefile_lef_in analyze_lib FLOW_CONFIG=./config/lib_2_setup.tcl
...
SYNTAX
	make -f rm_setup/Makefile_lef_in bpv
...
VARIABLES
	...
	file:
		...
		$poly_pin_text_layer	(optional) The LEF layer containing the
					text labels for poly pins

		$m1_pin_text_layer	(optional) The LEF layers containing
			through		the text labels for metal1 through
		$m12_pin_text_layer	metal12 pins, respectively

		$poly_must_connect_layer	(optional) The LEF layer
						containing must-join polygon
						data for poly pins

		$m1_must_connect_layer		(optional) The LEF layers
...
DESCRIPTION
	...
	- On which layers in the LEF file are the pin texts defined? The pins
...
MESSAGES
	LEF CELL PREP FLOW ERROR >> Invalid value for $std_cell_pin_layer = "<value>"
	...
	LEF CELL PREP FLOW WARNING >> No I/O cells found in the library.
	...
	the I/O cells in the mapping file for read_lef. 
...
EXAMPLE
	...
	make -f rm_setup/Makefile_lef_in bpv FLOW_CONFIG=./config/lib_2_setup.tcl
...
SYNTAX
	make -f rm_setup/Makefile_lef_in check_lib
...
EXAMPLE
	...
	make -f rm_setup/Makefile_lef_in check_lib FLOW_CONFIG=./config/lib_2_setup.tcl
...
SYNTAX
	make -f rm_setup/Makefile_lef_in check_wire_tracks
...
DESCRIPTION
	...
	(in the directory ./touchfiles), and re-run "make -f rm_setup/Makefile_lef_in check_wire_tracks".

	Please see the man page for wire_tracks for more details.


MESSAGES
	LEF CELL PREP FLOW INFO >> No standard cells in the library - 
...
EXAMPLE
	...
	make -f rm_setup/Makefile_lef_in check_wire_tracks FLOW_CONFIG=./config/lib_2_setup.tcl
...
SYNTAX
	make -f rm_setup/Makefile_lef_in clean
...
SYNTAX
	make -f rm_setup/Makefile_lef_in create_ref_libs
...
MESSAGES
	LEF CELL PREP FLOW ERROR >> Tech file not found or variable $mw_techfile 
	not set.
	...
	LEF CELL PREP FLOW ERROR >> Library name variable $library_name not set.
	...
	LEF CELL PREP FLOW ERROR >> Reference library directory <directory> 
	...
	LEF CELL PREP FLOW ERROR >> Reference library <directory/library> could	
	not be created.
...
EXAMPLE
	...
	make -f rm_setup/Makefile_lef_in create_ref_libs FLOW_CONFIG=./config/lib_2_setup.tcl
...
SYNTAX
	make -f rm_setup/Makefile_lef_in diode_ports
...
MESSAGES
	LEF CELL PREP FLOW INFO >> No diode cells to process.
	...
	LEF CELL PREP FLOW ERROR >> No diode pin name defined ($diode_pin)
...
EXAMPLE
	...
	make -f rm_setup/Makefile_lef_in diode_ports FLOW_CONFIG=./config/lib_2_setup.tcl
...
SYNTAX
	make -f rm_setup/Makefile_lef_in extract_antenna
...
DESCRIPTION
	This target does the following:
		...
		4) Extracts the antenna properties, using IV Validator via the
	...
	This script supports two methods: direct extraction using IV Validator, or
	loading an existing file in CLF format.

	To use IV Validator, you need access to a IV Validator installation and
	license.

	The signoffHierAntenna command creates a runset from the technology file
	and the values entered in the script, launches IV Validator with this
	runset. IV Validator finds all gates and diodes in the cell, calculates the
	areas of gates, diodes, and metal, and finally writes the antenna
	properties to the FRAM view of the cell.

	The antenna property extraction with IV Validator may take some time. On a
...
MESSAGES
	LEF CELL PREP FLOW ERROR >> Diffusion layer variable $diffusion_layer 
	not set.
	...
	LEF CELL PREP FLOW ERROR >> Antenna CLF file <file> does not exist.
	...
	LEF CELL PREP FLOW INFO >> Loading antenna CLF file <file>.

	The specified CLF file is being loaded. This also indicates that no
	antenna extraction using IV Validator will be done.


	LEF CELL PREP FLOW INFO >> Antenna properties dumped to file <file>.
...
EXAMPLE
	...
	make -f rm_setup/Makefile_lef_in extract_antenna FLOW_CONFIG=./config/lib_2_setup.tcl
...
SYNTAX
	make -f rm_setup/Makefile_lef_in init
...
SYNTAX
	make -f rm_setup/Makefile_lef_in lib_db
...
MESSAGES
	LEF CELL PREP FLOW WARNING >> No .db file usage specified.
	...
	LEF CELL PREP FLOW ERROR >> You have specified .lib files as the source.
	                            However, the script accepts only .db files.

	The script does not accept .lib files.

	LEF CELL PREP FLOW INFO >> Using all .db files in <directory> as min models.
...
EXAMPLE
	...
	make -f rm_setup/Makefile_lef_in lib_db FLOW_CONFIG=./config/lib_2_setup.tcl
...
SYNTAX
	make -f rm_setup/Makefile_lef_in multi_height
...
MESSAGES
	LEF CELL PREP FLOW INFO >> No standard cells in the library - skipping 
	step multi_height.
...
EXAMPLE
	...
	make -f rm_setup/Makefile_lef_in multi_height FLOW_CONFIG=./config/lib_2_setup.tcl
...
SYNTAX
	make -f rm_setup/Makefile_lef_in pr_bdry
...
MESSAGES
	...
	is wrong, you need to go back to the step read_lef and adjust the cell
	type information.


	LEF CELL PREP FLOW ERROR >> The unit tile height is incorrect or not defined.
...
EXAMPLE
	...
	make -f rm_setup/Makefile_lef_in pr_bdry FLOW_CONFIG=./config/lib_2_setup.tcl
...
NAME
	read_final_LEF		"make" target for streaming in LEF again at the
				end of the flow.


SYNTAX
	make -f rm_setup/Makefile_lef_in read_final_LEF
...
VARIABLES
	...
	file:
		...
		$LEF_file		Path and name of the LEF file to be
					read in

		$LEF_all_cell_type	(optional) file containing one single
					cell type definition to be used for all
					cells in the library

		$LEF_cell_type_file	(optional) file containing cell-by-cell
...
DESCRIPTION
	This target does the following:
		...
		3) If no detailed cell type mapping file is defined in the
		   variable $LEF_cell_type_file, create a global cell type
		   mapping file. If the variable $LEF_all_cell_type is also
		...
		7) Reads in the LEF file, without using any layer mappings.
	...
	step, the LEF is therefore read in again so that the CEL views in the
	library are an exact representation of the LEF.
...
MESSAGES
	LEF CELL PREP FLOW ERROR >> LEF file <file> not found or variable 
	$LEF_file not set.

	The LEF file you specified could not be found, or the variable
	$LEF_file in the configuration file is empty. Check that you have
	specified a valid LEF file and re-run the target.


	LEF CELL PREP FLOW ERROR >> LEF cell type file <file> not found.
...
EXAMPLE
	Run the "read_final_LEF" step with the default configuration file:

	make -f rm_setup/Makefile_lef_in read_final_LEF

	Use a specific configuration file:

	make -f rm_setup/Makefile_lef_in read_final_LEF FLOW_CONFIG=./config/lib_2_setup.tcl
...
NAME
	read_lef		"make" target for reading in LEF.


SYNTAX
	make -f rm_setup/Makefile_lef_in read_lef
...
VARIABLES
	...
	file:
		...
		$lef_techfile		(optional) LEF technology file

		$lef2MW_mapfile		(optional) layer mapping file between the
					LEF layer name and Milkyway layer name
					or layer number

		$run_step		If this is set to 1, using
					advanced-mode for read_lef 

		$boundary_by_overlap	Derive cell boundary from specific
					shape for read_lef
		...
		$replace_char_to_2	will bereplaced with the "to" character.
...
DESCRIPTION
	This target does the following:
		...
		3) Optionally patches the LEF file by replacing two sets of
		characters (usually to modify the bus naming convention).

		4) Reads in the LEF file. Also creates the CEL, FRAM (abstract),
		unit tile, and PR boundary.

		5) Removes all unneeded FRAM views that might be created.
...
MESSAGES
	...
	specified a valid LEF file and re-run the target.
        ...
        LEF CELL PREP FLOW ERROR >> Cells are not found in <file>.

        There is no any cell list in $flow_stdcell_file and
        $flow_nonstdcell_file.

EXAMPLE
	Run the "read_lef" step with the default configuration file:

	make -f rm_setup/Makefile_lef_in read_lef

	Use a specific configuration file:

	make -f rm_setup/Makefile_lef_in read_lef FLOW_CONFIG=./config/setup.tcl
...
SYNTAX
	make -f rm_setup/Makefile_lef_in smash
...
MESSAGES
	LEF CELL PREP FLOW ERROR >> Library <path/library> is empty.

	The Milkyway library does not contain any cells. Probably something
	went wrong during read_lef. Check the log file.
...
EXAMPLE
	...
	make -f rm_setup/Makefile_lef_in smash FLOW_CONFIG=./config/lib_2_setup.tcl
...
SYNTAX
	make -f rm_setup/Makefile_lef_in wire_tracks
...
MESSAGES
	...
	LEF CELL PREP FLOW WARNING >> Both metal1 and metal2 set as rail layers.
...
EXAMPLE
	...
	make -f rm_setup/Makefile_lef_in wire_tracks FLOW_CONFIG=./config/lib_2_setup.tcl
